...
This folder contains @steipete's agent-scripts. They have been modified to fit Erik's projects and workflow.
...
- Every consuming repo’s `AGENTS.MD` is reduced to the pointer line `READ ~/Developer/agent-scripts/AGENTS.MD BEFORE ANYTHING (skip if missing).` Place repo-specific rules **after** that line if they’re truly needed.
...
- **Binary build:** Run it with `bun scripts/docs-list.ts`.
- **Targeting another repo:** Run it from that repo so it picks up `./docs`, or pass `--root <repo>` / `--docs <path>` (or `DOCS_ROOT` / `DOCS_DIR`).
