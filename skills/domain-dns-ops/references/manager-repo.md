...
- `bin/namecheap-set-ns`: Namecheap NS flip helper (env vars in `~/Developer/manager/profile`).
- `bin/cloudflare-ai-bots`: bot management helper (needs token perms).

## Fast navigation

- Find a domain: `rg -n \"\\bexample\\.com\\b\" ~/Developer/manager/DOMAINS.md`
- List scripts: `ls -la ~/Developer/manager/bin`
